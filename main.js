...
/* -------- IGDB OAuth token (cached in electron-store) ------------------- */
...
/* -------- Cover helper (manual → cache → IGDB) -------------------------- */
async function igdbCoverURL (rawId) {
  ...
  for (const ext of ['.jpg', '.png']) {
    ...
    try { await fs.access(manual); return `file:///${manual.replace(/\\/g, '/')}`; }
    catch {}
  }
  ...
  if (coverIndex[rawId]) {
    ...
    try { await fs.access(cached); return `file:///${cached.replace(/\\/g, '/')}`; }
    catch {}
  }
  ...
  coverIndex[rawId] = imageId;           // add to index
  store.set('coverIndex', coverIndex);
  return `file:///${filePath.replace(/\\/g, '/')}`;
}

/* ──────────────────────────────────────────────────────────────────────────
   Discover installed Vortex games
   ──────────────────────────────────────────────────────────────────────── */
...
async function dirStats (dir) {
  ...
  for await (const rel of fg.stream(['**/*'], {
    cwd: dir, dot: true, onlyFiles: false
  })) {
...
/*  Browse for arbitrary folder and remember it as “custom game” */
...
/* open settings window (frameless) */
...
ipcMain.handle('covers:clear', async () => {
  ...
  store.set('coverIndex', {});
  return true;
});

ipcMain.handle('open-external', (_e, url) => shell.openExternal(url));

/* Mini file-tree streaming (capped) */
